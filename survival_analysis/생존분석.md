...
- 생존분석은 주로 의학통계에서 많이 사용해왔다. 
- 마케팅쪽에서는 주로 고객이탈(churn)과 관련하여 방법론을 가져와서 사용하기도 한다. 그외에도 특정한 treatment를 취한 뒤에 시간의 흐름에 따른 변화를 보기 위해서 많이 사용된다. 
- 이름이 생존분석이라 엄청 어려워보이지만 실제로는 새로운 개념 몇가지만 이해한다면 regression처럼 어렵지 않게 이해할 수 있다. 물론 딥하게 들어가면 어렵다 ^^;
- Python, R에서 사용할 수 있는 라이브러리들이 잘 되어있다.

---

#### 생존분석
- target data: 어떤 사건이 발생할 때까지 걸리는 시간 (time until an event occurs)
    - 일반적인 regression 문제 아닌가? censored data가 존재하기 때문에 생존분석 접근이 필요하다.
    - censored data? 예를 들어, 신약을 두 환자그룹에 투여하고 사망까지 걸리는 시간을 측정하고자 한다. 그런데 실험대상이던 환자가 외국으로 떠나서 연락이 두절되었다. 그러면 그 환자는 실제로 사망한 것은 아니지만 실제로 살았는지 죽었는지 알 방법이 없고 데이터를 더 이상 얻을 수 없는 시점을 그 환자의 수명으로 정할 수 밖에 없다. 이처럼 우리가 기대하는 이벤트(여기서는 사망)가 아닌 다른 이유로 특정 시점 이후로 해당 정보를 얻을 수 없는 데이터를 censored data라고 한다.
...
- 생존시간에 대한 변수들의 효과 파악
    - Cox 회귀 모형
---
그렇다면 이번 세미나를 통해 다룰 내용(=기억하면 좋은 내용)은
- 개념
    - 생존함수, 위험도함수
- 방법론
    - Kaplan-Meier
    - log-rank test
    - cox's proportional harzad model
...
생존함수를 통해 알 수 있는 정보는? 예를 들어, 이번 A 마케팅해보니까 4주이상 고객들이 이탈하지 않을 확률이 20%군요!
...
#### log-rank test
- 일관적인 차이를 검출한다.
    - 여기서 일관적인이란 두 그룹의 survival curve를 그렸을 때, 서로 교차하지 않는 것을 의미한다.
    - 교차하게 되면 검정할 수 없다.
